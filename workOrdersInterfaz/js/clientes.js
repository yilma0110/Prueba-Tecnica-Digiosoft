...
//Ejecuta la petición para eliminar un cliente seleccionado
async function eliminarCliente(idSelected){
    try{
        let options = {
            method:"DELETE",
            headers: {
        ...
        const resp = await fetch(UrlApi + "clientes/eliminar/" + idSelected, options);
...
dataTable_clientes.addEventListener("click", (e) => {
    ...
    if (e.target.name == 'editar') {
        ...
        Swal.fire({
            title: `¿Estás seguro de eliminar el cliente con ID ${idSelected} ?`,
